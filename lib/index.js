...
export function loop(condition, middleware) {
  return (deps) => async (...params) => {
    ...
    const queueTail = params.slice(1)
    while (condition(tmpParam, index++, ...queueTail)) {
      tmpParam = await middleware(deps)(tmpParam, ...queueTail)
    }
    return tmpParam
  }
}
...
export function ask(index) {
  return (deps) => (param, queue) => queue[index - 1] || undefined
}
